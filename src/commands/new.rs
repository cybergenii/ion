...
fn create_library_template(base_path: &Path, name: &str) -> Result<()> {
    let header = format!(r#"#ifndef {guard}_H
#define {guard}_H
...
#endif // {guard}_H
"#, name = name, guard = name.to_uppercase());
    
    fs::write(base_path.join("include").join(format!("{}.h", name)), header)?;
    
    let source = format!(r#"#include "{name}.h"
